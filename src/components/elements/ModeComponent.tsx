...
const ModeComponent: FC<ModeProps> = ({ settings: set, path, section }) => {
  ...
  const { title, modes, hint, tags } = set;
  ...
  const drop = !tags?.includes("no_dropdown");
  ...
  return (
    <>
                ...
                {Object.entries(modes).map(
                  ([value, { title: label, unit }], idx: number) => {
                    const btnStyle = getButtonStyle(
                      value
                        ? (set as ModeSetting).modes[value]?.tags
                        : undefined,
      ...
      <Flex direction="row" margin={drop ? "0.2rem 0.1rem 0.4rem 0.3rem" : ""}>
        {drop && (
          <Center>
            <Divider
